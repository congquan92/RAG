...
import { Search, ChevronDown, ChevronUp, ArrowRight, Users, Building2, MapPin, Lightbulb, Calendar, Tag, Loader2, Network } from "lucide-react";
...
function RelationshipRow({ rel, entityName }: { rel: KGRelationship; entityName: string }) {
    ...
    return (
        <div className="flex items-center gap-2 text-xs py-1.5 px-3">
            <span className={cn("font-medium truncate max-w-[140px]", isSource ? "text-foreground" : "text-muted-foreground")}>{isSource ? entityName : other}</span>
            <ArrowRight className="w-3 h-3 text-muted-foreground/50 flex-shrink-0" />
            <span className="text-muted-foreground/70 truncate max-w-[160px] italic">{rel.description || rel.keywords || "liên quan"}</span>
            <ArrowRight className="w-3 h-3 text-muted-foreground/50 flex-shrink-0" />
            <span className={cn("font-medium truncate max-w-[140px]", isSource ? "text-muted-foreground" : "text-foreground")}>{isSource ? other : entityName}</span>
        </div>
    );
}
...
// ---------------------------------------------------------------------------
const EntityRow = memo(function EntityRow({ entity, projectId }: { entity: KGEntity; projectId: string }) {
    ...
    return (
        <div className="border-b last:border-b-0">
            <button onClick={() => setExpanded(!expanded)} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-muted/50 transition-colors text-left" data-entity-name={entity.name}>
                {/* Name */}
                ...
                {/* Degree */}
                <span className="text-xs text-muted-foreground/60 w-8 text-right flex-shrink-0">{entity.degree}</span>

                {/* Expand */}
                {entity.degree > 0 && (expanded ? <ChevronUp className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0" /> : <ChevronDown className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0" />)}
            </button>
                ...
                {expanded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                        <div className="px-3 pb-3 space-y-2">
                            {/* Description */}
                            {entity.description && <p className="text-xs text-muted-foreground/80 leading-relaxed pl-1">{entity.description}</p>}
                            ...
                            {relationships && relationships.length === 0 && <p className="text-xs text-muted-foreground/50 pl-1">Không tìm thấy mối quan hệ</p>}
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
    ...
    if (!entities || entities.length === 0) {
        return (
            <div className="flex flex-col items-center py-10 text-center">
                ...
                <p className="text-sm text-muted-foreground">Chưa có entity nào được trích xuất</p>
                <p className="text-xs text-muted-foreground/60 mt-1">Entity sẽ được trích xuất tự động khi tài liệu được xử lý bằng NexusRAG</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="flex gap-2 flex-wrap">
                ...
                {/* Type filter */}
                <select value={typeFilter ?? ""} onChange={(e) => setTypeFilter(e.target.value || null)} className="h-8 px-2 rounded-md border border-input bg-background text-xs">
                ...
                {/* Sort */}
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortKey)} className="h-8 px-2 rounded-md border border-input bg-background text-xs">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs text-muted-foreground">{filtered.length} entity</span>
                {typeFilter && (
                    <button onClick={() => setTypeFilter(null)} className="text-[10px] text-primary bg-primary/10 px-2 py-0.5 rounded-full hover:bg-primary/20 transition-colors">
                        {typeFilter} &times;
                    </button>
                )}
            </div>
            ...
            <div className="rounded-lg border divide-y max-h-[500px] overflow-y-auto">
                {filtered.map((entity) => {
                    const isHighlighted = highlightEntities.some((e) => e.toLowerCase() === entity.name.toLowerCase());
                    return (
                        <div key={entity.name} className={cn(isHighlighted && "bg-amber-400/10 border-l-2 border-l-amber-400")}>
            ...
            {filtered.length === 0 && entities.length > 0 && <p className="text-center text-xs text-muted-foreground py-4">Không có entity nào khớp bộ lọc</p>}
        </div>
    );
});
