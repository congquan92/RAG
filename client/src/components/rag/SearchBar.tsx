        ...
        <div className="space-y-3">
            ...
            <div className="flex gap-2">
                <div className="relative flex-1">
                    ...
                    />
                    {/* Shortcut hint */}
                    {!query && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground/50 font-mono pointer-events-none">/</span>}
                    {query && (
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                            {MODES.map((m) => (
                                <button
                                    ...
                                    title={m.description}
                                    className={cn("px-2.5 py-1 text-xs font-medium rounded-md transition-colors", mode === m.value ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground")}
                                >
                                    {m.label}
                                </button>
                            ))}
                                ...
                                {indexedDocs.map((d) => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            selectedDocs.includes(d.id) ? "bg-primary/15 text-primary border border-primary/30" : "bg-muted text-muted-foreground hover:text-foreground",
                                        )}
                                    >
                                        {d.original_filename}
                                    </button>
