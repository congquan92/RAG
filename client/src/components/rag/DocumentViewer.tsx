...
// ---------------------------------------------------------------------------
const TOCSidebar = memo(function TOCSidebar({ headings, activeId, onSelect }: { headings: Heading[]; activeId: string | null; onSelect: (id: string) => void }) {
                ...
                {headings.map((h) => (
                    <li key={h.id}>
                        <button
                            ...
                            className={cn("w-full text-left text-xs py-1 px-2 rounded-md transition-colors truncate", "hover:bg-muted", activeId === h.id ? "text-primary font-medium bg-primary/10" : "text-muted-foreground")}
...
function insertPageDividers(markdown: string): string {
    ...
    return markdown.replace(/(?:<!--\s*page\s+(\d+)\s*-->|(?:^|\n)---+\s*\n+(?=##?\s))/gi, (match, pageNo) => {
...
export const DocumentViewer = memo(function DocumentViewer({ doc, scrollToPage, scrollToHeading, scrollToImageSrc, highlightChunks, onScrolled }: DocumentViewerProps) {
    ...
    const {
        ...
        // Server contract currently has no GET /documents/{id}/markdown endpoint.
        enabled: false,
    ...
    const mdComponents = useMemo<import("react-markdown").Components>(
        () => ({
            ...
            p: ({ children, node, ...props }) => {
                const hasImage = (node as any)?.children?.some((child: any) => child.type === "element" && child.tagName === "img");
                if (hasImage)
                    return (
            ...
            img: ({ src, alt, ...props }) => (
                <figure className="my-4">
                    <img
                        ...
                        {...props}
                    />
                    {alt && <figcaption className="text-xs text-muted-foreground text-center mt-1.5 italic">{alt}</figcaption>}
                </figure>
            ),
        }),
        [],
    ...
    const scrollTo = useCallback((target: HTMLElement, block: "start" | "center" = "center", onDone?: () => void) => {
    ...
    useEffect(() => {
        ...
        const rafId = requestAnimationFrame(() =>
            requestAnimationFrame(() => {
                ...
                if (scrollToImageSrc) {
                    const imgEl = contentRef.current.querySelector(`img[src="${CSS.escape(scrollToImageSrc)}"]`) as HTMLElement | null;
                    if (imgEl) {
                    ...
                    if (scrollToPage) {
                        const pageEl = contentRef.current.querySelector(`[data-page="${scrollToPage}"]`) as HTMLElement | null;
                        if (pageEl) {
                ...
                if (scrollToHeading) {
                    const targetId = scrollToHeading
                        ...
                        .replace(/\s+/g, "-")
                        .slice(0, 80);
                    const el = contentRef.current.querySelector(`#${CSS.escape(targetId)}`) as HTMLElement | null;
                    if (el) {
    ...
    useEffect(() => {
        ...
        for (const chunk of highlightChunks) {
            ...
            const lastHeading = chunk.heading_path.length > 0 ? chunk.heading_path[chunk.heading_path.length - 1] : null;
    ...
    return (
        <div className="flex h-full min-h-0">
            {/* TOC sidebar */}
            {showToc && <TOCSidebar headings={headings} activeId={activeHeading} onSelect={handleTocSelect} />}
            ...
            <div ref={contentRef} className="flex-1 min-h-0 overflow-y-auto">
                ...
                {headings.length > 0 && (
                    <button
                        ...
                        className={cn("sticky top-2 left-2 z-10 p-1.5 rounded-md border bg-background/80 backdrop-blur-sm", "hover:bg-muted transition-colors xl:hidden", "flex items-center gap-1 text-xs text-muted-foreground")}
                    >
