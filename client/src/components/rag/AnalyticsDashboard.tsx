...
// Stat card
// ---------------------------------------------------------------------------
function StatCard({ icon: Icon, label, value, accent, className }: { icon: typeof FileText; label: string; value: number | string; accent?: boolean; className?: string }) {
    return (
    ...
    return (
        <div className="space-y-2">
            <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-muted-foreground">Phân tích theo tài liệu</span>
            </div>
            ...
            <div className="space-y-1.5 max-h-[240px] overflow-y-auto">
                {docs.map((doc, i) => {
                    ...
                    return (
                        <motion.div key={doc.document_id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="space-y-0.5">
                            ...
                            <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
                                <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.4, delay: i * 0.03 }} className={cn("h-full rounded-full", STATUS_COLOR[doc.status] ?? "bg-muted-foreground")} />
                            </div>
                        </motion.div>
                    );
                })}
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center py-8">
                ...
                <span className="text-sm text-muted-foreground">Đang tải thống kê...</span>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-5">
            ...
            <div className={cn("grid gap-2", compact ? "grid-cols-3" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-6")}>
                ...
                <StatCard icon={Database} label="Đã index" value={stats.indexed_documents} accent />
            ...
            {kg_analytics && kg_analytics.entity_count > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div className="rounded-lg border bg-card/60 p-4 space-y-2">
                        <span className="text-xs font-medium text-muted-foreground">Entity nổi bật</span>
