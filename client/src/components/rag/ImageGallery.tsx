...
import { Image, ImageOff, ChevronDown, ChevronUp, X, ChevronLeft, ChevronRight } from "lucide-react";
...
function GalleryEmpty() {
    return (
        <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            ...
            <p className="text-sm text-muted-foreground">Không có hình ảnh được trích xuất</p>
            <p className="text-xs text-muted-foreground/60 mt-1">Tài liệu này không chứa hình ảnh có thể trích xuất</p>
        </div>
    );
}
...
// Lightbox
// ---------------------------------------------------------------------------
function GalleryLightbox({ images, currentIndex, onClose, onNavigate }: { images: DocumentImage[]; currentIndex: number; onClose: () => void; onNavigate: (index: number) => void }) {
    ...
    return (
        ...
        >
            {/* Close */}
            <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors text-white z-10">
                <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex gap-6 max-w-[90vw] max-h-[85vh]" onClick={(e) => e.stopPropagation()}>
                <img src={img.url} alt={img.caption || `Hình từ trang ${img.page_no}`} className="max-w-full max-h-[80vh] rounded-lg object-contain" />
        ...
        <div ref={imgRef} className={className} onClick={onClick}>
            {isVisible ? (
                <>
                    ...
                    <img src={src} alt={alt} className={cn("w-full h-full object-cover transition-opacity duration-300", loaded ? "opacity-100" : "opacity-0")} loading="lazy" onLoad={() => setLoaded(true)} />
                </>
            ) : (
...
// Page group section
// ---------------------------------------------------------------------------
const PageSection = memo(function PageSection({ group, allImages, onImageClick }: { group: PageGroup; allImages: DocumentImage[]; onImageClick: (globalIndex: number) => void }) {
    const [collapsed, setCollapsed] = useState(false);

    return (
        <div className="space-y-2">
            {/* Page header */}
            <button onClick={() => setCollapsed(!collapsed)} className="flex items-center gap-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors w-full">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {group.images.map((img) => {
                                ...
                                return (
                                    ...
                                    >
                                        <LazyImage src={img.url} alt={img.caption || `Trang ${img.page_no}`} className="w-full h-full relative" />
                                        {/* Hover overlay */}
                                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-end">
                                            <div className="p-1.5 w-full translate-y-full group-hover:translate-y-0 transition-transform">
                                                <p className="text-[10px] text-white/90 truncate">{img.caption || `Hình ${img.image_id.slice(0, 8)}`}</p>
                                            </div>
                                        </div>
                                    </motion.div>
                                );
    ...
    return (
        <div className="p-6 space-y-4">
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">trên {pageGroups.length} trang</span>
            </div>
            ...
            {pageGroups.length === 1 ? (
                ...
                <div className="space-y-4">
                    {pageGroups.map((group) => (
                        <PageSection key={group.pageNo} group={group} allImages={allImages} onImageClick={handleImageClick} />
                    ))}
                </div>
            )}

            {/* Lightbox */}
            <AnimatePresence>{lightboxIndex !== null && <GalleryLightbox images={allImages} currentIndex={lightboxIndex} onClose={() => setLightboxIndex(null)} onNavigate={setLightboxIndex} />}</AnimatePresence>
        </div>
    );
});
