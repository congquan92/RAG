...
// Image lightbox
// ---------------------------------------------------------------------------
function ImageLightbox({ images, currentIndex, onClose, onNavigate }: { images: DocumentImage[]; currentIndex: number; onClose: () => void; onNavigate: (index: number) => void }) {
    ...
    return (
        <motion.div
            ...
            role="dialog"
        >
            <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors text-white">
                <X className="w-5 h-5" />
            </button>
...
export const ImageResultGrid = memo(function ImageResultGrid({ images }: ImageResultGridProps) {
    ...
    return (
        <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                {images.map((img, i) => (
                    <motion.div
                        ...
                        className={cn("group relative rounded-lg overflow-hidden bg-muted cursor-pointer", "aspect-video")}
                        onClick={() => setLightboxIndex(i)}
                    >
                        <img src={img.url} alt={img.caption || `Trang ${img.page_no}`} className="w-full h-full object-cover" loading="lazy" />
                        {/* Hover overlay */}
            ...
            {/* Lightbox */}
            <AnimatePresence>{lightboxIndex !== null && <ImageLightbox images={images} currentIndex={lightboxIndex} onClose={() => setLightboxIndex(null)} onNavigate={setLightboxIndex} />}</AnimatePresence>
        </div>
    );
});
