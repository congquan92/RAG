...
import { Send, Square, Bot, User, Loader2, Trash2, Sparkles, FileText, Save, ImageIcon, Brain, ChevronDown, Settings, RotateCcw, Info, Copy, ClipboardCheck, FileCode, ThumbsUp, ThumbsDown, DatabaseZap } from "lucide-react";
...
function useFindDoc(documentId: string): Document | undefined {
            ...
            parts.forEach((part, i) => {
                ...
                tokens.forEach((token, ti) => {
                    ...
                    if (imgMatch && imageRefs && imageRefs.length > 0) {
                        ...
                        const imageRef = imageRefs.find((ir) => ir.ref_id === imgId) ?? imageRefs[parseInt(imgId, 10) - 1]; // legacy 1-indexed
                        if (imageRef) {
                    ...
                    const source = sources.find((s) => String(s.index) === token) ?? (fallbackSources ? fallbackSources.find((s) => String(s.index) === token) : undefined);
                    if (source) {
...
// ---------------------------------------------------------------------------
function MarkdownWithCitations({ content, sources, relatedEntities, imageRefs }: { content: string; sources: ChatSourceChunk[]; relatedEntities: string[]; imageRefs?: ChatImageRef[] }) {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={cn("flex gap-2", isUser ? "justify-end" : "justify-start")}>
            ...
            <div className={cn(isUser ? "max-w-[85%] rounded-xl px-3 py-2 bg-secondary/50" : "max-w-[90%] min-w-0 py-1")}>
                ...
                {!isUser && message.thinking && !message.isStreaming && !message.agentSteps?.some((s) => s.thinkingText) && <ThinkingPanel thinking={message.thinking} />}
...
export const ChatPanel = memo(function ChatPanel({ workspaceId, hasIndexedDocs, workspace }: ChatPanelProps) {
    ...
    const handleSavePrompt = useCallback(() => {
        toast.info("Server does not support saving custom prompts yet.");
    }, []);

    const handleResetPrompt = useCallback(() => {
        setPromptDraft(DEFAULT_SYSTEM_PROMPT);
        toast.info("Prompt reset is local-only because server has no prompt settings endpoint.");
    }, []);

    // Current server does not expose /rag/capabilities.
    ...
    useEffect(() => {
        ...
        setMessages((prev) => {
            ...
            if (m.content === newContent && m.sources === newSources && m.imageRefs === newImages && m.thinking === newThinking && m.agentSteps === newSteps) {
    ...
    const handleSend = useCallback(
        async (text?: string) => {
            ...
            const finalMsg = await stream.sendMessage(msg, history, thinkingSupported && enableThinking, forceSearch);
                    ...
                    prev.map((m) =>
                        m.id === assistantId
                            ? {
                                  ...
                                  id: assistantId,
                                  isStreaming: false,
                                  agentSteps: finalMsg.agentSteps?.length ? finalMsg.agentSteps : agentStepsRef.current.length > 0 ? agentStepsRef.current : m.agentSteps,
                              }
                            : m,
                    ),
                );
