...
function RelevanceBar({ score }: { score: number }) {
    ...
    return (
        <div className="h-1 w-full rounded-full bg-muted overflow-hidden">
            <div className={cn("h-full rounded-full transition-all duration-500", pct >= 70 ? "bg-primary" : pct >= 40 ? "bg-amber-400" : "bg-destructive/60")} style={{ width: `${pct}%` }} />
        </div>
    );
}
...
export const ResultCard = memo(function ResultCard({ chunk, index, query }: ResultCardProps) {
    ...
    return (
        <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="group rounded-lg border bg-card/60 hover:bg-card transition-colors overflow-hidden">
            <div className="px-4 py-3 space-y-2">
                ...
                <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                        ...
                        {citation?.heading_path && citation.heading_path.length > 0 && <span className="text-xs text-muted-foreground/50 truncate hidden sm:inline">{citation.heading_path.join(" > ")}</span>}
                    </div>
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                        <span className="text-[10px] font-medium text-muted-foreground/60">{(similarity * 100).toFixed(0)}%</span>
                        <button onClick={handleCopy} className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-muted" title="Sao chép nội dung">
