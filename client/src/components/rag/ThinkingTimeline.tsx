...
import { Brain, Lightbulb, Search, Database, PenLine, CheckCircle2, AlertCircle, Loader2, ChevronDown, type LucideIcon } from "lucide-react";
...
const STEP_CONFIG: Record<AgentStepType, StepConfig> = {
    ...
    generating: { icon: PenLine, label: "Đang tạo phản hồi" },
...
function LiveTimer({ startTimestamp }: { startTimestamp: number }) {
    ...
    return <span className="text-[11px] font-mono tabular-nums text-primary/80">{formatMs(elapsed)}</span>;
}
...
function ThinkingLogSection({ text }: { text: string }) {
    ...
    return (
        <div className="mt-1.5">
            <button onClick={() => setExpanded((p) => !p)} className="flex items-center gap-1 text-[11px] text-muted-foreground/70 hover:text-muted-foreground transition-colors">
                <Brain className="w-2.5 h-2.5" />
                <span>{expanded ? "Ẩn" : "Hiện"} log suy nghĩ</span>
                <ChevronDown className={cn("w-2.5 h-2.5 transition-transform", expanded && "rotate-180")} />
            </button>
            <AnimatePresence>
                {expanded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                        <div className={cn("mt-1 ml-1 text-[11px] leading-relaxed text-muted-foreground/80 italic", "max-h-[200px] overflow-y-auto scrollbar-none", "border-l border-border/40 pl-2", "whitespace-pre-wrap break-words")}>{text}</div>
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
...
function StepNode({ step, isLast, isLive }: StepNodeProps) {
    ...
    return (
        <motion.div initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, ease: "easeOut" }} className="flex gap-2 relative">
            {/* Vertical line connector */}
            {!isLast && <div className={cn("absolute left-[9px] top-[20px] w-px bottom-0", isActive ? "bg-primary/20" : "bg-border/50")} />}
                ...
                <div className="flex items-center gap-1.5 min-h-[18px]">
                    ...
                    <span className="ml-auto flex-shrink-0">
                        {isActive && isLive ? (
                            ...
                            <span className="text-[11px] font-mono tabular-nums text-muted-foreground/70">{formatMs(step.durationMs)}</span>
                        ) : null}
                    </span>
                </div>
                ...
                {step.sourceBadges && step.sourceBadges.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                        {step.sourceBadges.map((badge) => (
                            <span key={badge} className="inline-flex items-center px-1 py-0.5 text-[10px] font-mono font-bold rounded bg-primary/10 text-primary/80">
                                {badge}
                            </span>
                        ))}
                    </div>
            ...
            handles display. After completion, show collapsible log here. */}
                {step.step === "analyzing" && step.thinkingText && !isActive && <ThinkingLogSection text={step.thinkingText} />}
            </div>
        </motion.div>
    );
}
...
function buildSummary(steps: AgentStep[]): string {
    ...
    if (sourcesStep) {
        ...
        if (sourcesStep.imageCount) {
            sourceText += ` + ${sourcesStep.imageCount} hình`;
        }
    ...
    if (sourcesStep) {
        const suffix = parts[1] ? ` trong ${parts[1]}` : activeStep ? " — đang tạo phản hồi..." : "";
        return `Tìm thấy ${parts[0]}${suffix}`;
    }
...
export function ThinkingTimeline({ steps, mode, className, autoCollapse = false }: ThinkingTimelineProps) {
    ...
    if (!expanded) {
        return (
            <div className={cn("rounded-md border border-border/60 bg-background overflow-hidden", className)}>
                <button onClick={() => setExpanded(true)} className="w-full flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-primary/80 hover:text-primary transition-colors">
                    {isStillActive ? <Loader2 className="w-3 h-3 animate-spin text-primary/80 flex-shrink-0" /> : <CheckCircle2 className="w-3 h-3 text-emerald-500/80 flex-shrink-0" />}
    ...
    return (
        <div className={cn("relative", isEmbedded && "rounded-md border border-border/60 bg-background overflow-hidden", className)}>
            ...
            {isEmbedded && (
                <button onClick={() => setExpanded(false)} className="w-full flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-primary/80 hover:text-primary transition-colors border-b border-border/40">
