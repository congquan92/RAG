...
function simulateForces(nodes: SimNode[], edges: KGGraphEdge[], width: number, height: number, alpha: number): void {
    ...
    return (
        <div className="relative w-full h-full">
            ...
            <div className="absolute top-2 right-2 z-10 flex flex-col gap-1">
                <button onClick={() => setZoom((z) => Math.min(3, z + 0.2))} className="p-1.5 rounded-md border bg-background/80 backdrop-blur-sm hover:bg-muted transition-colors">
                    <ZoomIn className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => setZoom((z) => Math.max(0.3, z - 0.2))} className="p-1.5 rounded-md border bg-background/80 backdrop-blur-sm hover:bg-muted transition-colors">
            ...
            >
                <g transform={`translate(${pan.x},${pan.y}) scale(${zoom})`}>
                    ...
                    {nodes.map((node) => {
                        ...
                        const isHighlighted = highlightEntities.length > 0 && highlightEntities.some((e) => e.toLowerCase() === node.label.toLowerCase());
                        const dimmed = highlightEntities.length > 0 ? !isHighlighted && !isHovered && !isSelected : (hoveredNode || selectedNode) && !connectedNodes.has(node.id);
                        ...
                        return (
                            ...
                            >
                                ...
                                {(isHovered || isSelected || isHighlighted) && (
                                    <circle r={r + (isHighlighted ? 6 : 4)} fill="none" stroke={isHighlighted ? "#fbbf24" : color} strokeWidth={isHighlighted ? 3 : 2} strokeOpacity={isHighlighted ? 0.7 : 0.4}>
                                        {isHighlighted && <animate attributeName="stroke-opacity" values="0.7;0.3;0.7" dur="2s" repeatCount="indefinite" />}
                                    </circle>
                                )}
                                {/* Node circle */}
                                <circle r={r} fill={color} fillOpacity={dimmed ? 0.15 : 0.85} stroke={color} strokeWidth={isSelected ? 2 : 1} strokeOpacity={dimmed ? 0.2 : 1} />
                                {/* Label (shown when not too zoomed out) */}
                                {zoom > 0.5 && (
                                    <text y={r + 12} textAnchor="middle" fontSize={10} fill="currentColor" fillOpacity={dimmed ? 0.15 : 0.7} className="pointer-events-none select-none">
                ...
                (() => {
                    ...
                    return (
                        <div className="absolute top-2 left-2 z-10 bg-background/95 backdrop-blur-sm border rounded-lg p-3 shadow-lg max-w-[220px]">
                            ...
                            <p className="text-xs text-muted-foreground/70 mt-1">{node.degree} kết nối</p>
                        </div>
                    );
                })()}

            {data.is_truncated && <div className="absolute bottom-2 right-2 z-10 text-[10px] text-amber-400 bg-background/80 backdrop-blur-sm border border-amber-400/30 rounded px-2 py-1">Đồ thị đã được rút gọn (quá nhiều node)</div>}
        </div>
    );
});
    ...
    if (!data || data.nodes.length === 0) {
        return (
            <div className="flex flex-col items-center py-10 text-center">
                ...
                <p className="text-xs text-muted-foreground/60 mt-1">Hãy xử lý tài liệu với NexusRAG để tạo Knowledge Graph</p>
            </div>
        );
    }
