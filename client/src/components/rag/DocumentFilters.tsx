...
export const DocumentFilters = memo(function DocumentFilters({ searchQuery, onSearchChange, statusFilter, onStatusChange, counts }: DocumentFiltersProps) {
    return (
                ...
                {TABS.map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            className={cn("px-2.5 py-1 text-xs font-medium rounded-md transition-colors", isActive ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}
                        >
                            {tab.label}
                            {count > 0 && <span className={cn("ml-1 text-[10px]", isActive ? "text-primary" : "text-muted-foreground/60")}>{count}</span>}
                        </button>
                    );
                })}
            </div>
