            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        className={cn("fixed top-0 right-0 h-full z-40", "bg-background border-l shadow-2xl", "flex flex-col", className)}
                    >
                        {/* Resize handle */}
                        <div onMouseDown={handleMouseDown} className="absolute left-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-primary/30 active:bg-primary/50 transition-colors z-50" />

                        {/* Header bar */}
                        <div className="flex items-center justify-between px-4 py-3 border-b flex-shrink-0">
                            <div className="flex-1 min-w-0">{header}</div>
                            <button onClick={onClose} className="ml-2 p-1.5 rounded-md hover:bg-muted transition-colors flex-shrink-0" title="Dong panel (Esc)">
                                <X className="w-4 h-4" />
                            </button>
                        </div>
            ...
            {open && (
                <>
                    ...
                    <motion.div
                        ...
                        exit={{ y: "100%" }}
                        transition={{ type: "spring", damping: 30, stiffness: 300 }}
                        className={cn("fixed inset-x-0 bottom-0 top-12 z-50", "bg-background rounded-t-xl shadow-2xl", "flex flex-col", className)}
                    >
                        {/* Header bar */}
                        <div className="flex items-center justify-between px-4 py-3 border-b flex-shrink-0">
                            <div className="flex-1 min-w-0">{header}</div>
                            <button onClick={onClose} className="ml-2 p-1.5 rounded-md hover:bg-muted transition-colors flex-shrink-0">
                                <X className="w-4 h-4" />
                            </button>
                        </div>
