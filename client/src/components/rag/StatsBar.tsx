...
function StatItem({ icon: Icon, label, value, accent }: { icon: typeof FileText; label: string; value: number | string; accent?: boolean }) {
    return (
        <div className="flex items-center gap-1.5">
            <Icon className={cn("w-3.5 h-3.5", accent ? "text-primary" : "text-muted-foreground")} />
            <span className="text-xs text-muted-foreground">{label}</span>
            <span className={cn("text-xs font-semibold", accent ? "text-primary" : "text-foreground")}>{value}</span>
        </div>
    );
}
        ...
        <div className="flex items-center gap-4 flex-wrap">
            ...
            {processingCount > 0 && (
                <div className="flex items-center gap-1.5 ml-auto">
                    ...
                    <span className="text-xs text-amber-400 font-medium">Đang xử lý {processingCount} tài liệu...</span>
                </div>
            )}
        </div>
    );
