...
export const Sidebar = memo(function Sidebar({ collapsed, onToggle }: SidebarProps) {
    ...
    const activeWorkspaceId = location.pathname.match(/\/knowledge-bases\/([^/]+)/)?.[1];
    const isHome = location.pathname === "/";

    return (
        <aside className={cn("flex flex-col h-full bg-card border-r border-border transition-all duration-200 flex-shrink-0", collapsed ? "w-14" : "w-60")}>
            {/* Logo */}
            <div className="flex items-center gap-2.5 px-3 h-12 border-b border-border flex-shrink-0">
                ...
                {!collapsed && <span className="font-bold text-primary text-base truncate">NexusRAG</span>}
            </div>
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            isActive ? "bg-primary/10 text-primary border-l-2 border-primary font-medium" : "text-muted-foreground hover:bg-muted/50 hover:text-foreground",
                                        )}
                                    >
                                        <Database className="w-3.5 h-3.5 flex-shrink-0" />
                                        <span className="truncate">{ws.name}</span>
                                        <span className="ml-auto text-[10px] text-muted-foreground/60 tabular-nums">{ws.document_count}</span>
                                    </button>
                                );
                            })}
                        </div>
                        ...
                        {workspaces?.slice(0, 6).map((ws) => {
                            ...
                            return (
                                <button
                                    key={`ws-${ws.id}`}
                                    onClick={() => navigate(`/knowledge-bases/${ws.id}`)}
                                    className={cn("w-full flex items-center justify-center py-1.5 rounded-lg transition-colors", isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted/50")}
                                    title={ws.name}
                                >
