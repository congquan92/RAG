...
export const ConfirmDialog = memo(function ConfirmDialog({ open, onConfirm, onCancel, title = "Xác nhận", message, confirmLabel = "Xác nhận", cancelLabel = "Hủy", variant = "default" }: ConfirmDialogProps) {
            ...
            >
                <div className="p-6">
                    ...
                    <div className="flex items-start gap-4">
                        <div className={cn("flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center", variant === "danger" ? "bg-destructive/15" : "bg-primary/15")}>
                            <AlertTriangle className={cn("w-5 h-5", variant === "danger" ? "text-destructive" : "text-primary")} />
                        </div>
                        <div className="flex-1 min-w-0">
                            <h3 id="confirm-title" className="text-lg font-semibold leading-tight">
                                {title}
                            </h3>
                            <p id="confirm-message" className="mt-2 text-sm text-muted-foreground leading-relaxed">
                                {message}
                            </p>
                        </div>
                    </div>
                ...
                <div className="flex justify-end gap-3 px-6 py-4 border-t border-border/50 bg-muted/20 rounded-b-xl">
                    ...
                    <Button variant={variant === "danger" ? "destructive" : "default"} size="sm" onClick={onConfirm}>
                        {confirmLabel}
                    </Button>
                </div>
            </div>
